...
export async function e2eDeriveKey(password: string, salt: Uint8Array): Promise<CryptoKey> {
  const base = await crypto.subtle.importKey('raw', e2eTextEncoder.encode(password), 'PBKDF2', false, ['deriveKey'])
...
export async function e2eSetup(password: string): Promise<E2ESetup> {
  ...
  const ct = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, e2eTextEncoder.encode(JSON.stringify(privJwk)))
...
export async function e2eImportPublic(pubB64: string): Promise<CryptoKey> {
  return crypto.subtle.importKey('raw', e2eUnb64(pubB64), { name: 'ECDH', namedCurve: 'P-256' }, false, [])
}
...
async function e2eSharedKey(priv: CryptoKey, pub: CryptoKey): Promise<CryptoKey> {
  ...
  return crypto.subtle.deriveKey(
    { name: 'HKDF', hash: 'SHA-256', salt: new Uint8Array(16), info: e2eTextEncoder.encode('KaltenGram-E2E') },
    hkdfKey,
    { name: 'AES-GCM', length: 256 },
    false,
    ['encrypt', 'decrypt']
...
export async function e2eNewMessageKey(): Promise<CryptoKey> {
  return crypto.subtle.importKey('raw', e2eRandomBytes(32), { name: 'AES-GCM' }, true, ['encrypt', 'decrypt'])
}
...
export async function e2eWrapMessageKey(mk: CryptoKey, recipientPubB64: string, myPriv: CryptoKey): Promise<Omit<WrappedKey, 'userId'>> {
  ...
  const ct = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, shared, exported)
...
export async function e2eUnwrapMessageKey(wrapped: Omit<WrappedKey, 'userId'>, senderPubB64: string, myPriv: CryptoKey): Promise<CryptoKey> {
  ...
  const shared = await e2eSharedKey(myPriv, pub)
  const pt = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: e2eUnb64(wrapped.iv) }, shared, e2eUnb64(wrapped.key))
  return crypto.subtle.importKey('raw', pt, { name: 'AES-GCM' }, true, ['encrypt', 'decrypt'])
}

export async function e2eEncryptText(text: string, mk: CryptoKey): Promise<EncryptedPayload> {
  const iv = e2eRandomBytes(12)
  const ct = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, mk, e2eTextEncoder.encode(text))
...
export async function e2eDecryptText(payload: EncryptedPayload, mk: CryptoKey): Promise<string> {
  const pt = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: e2eUnb64(payload.nonce) }, mk, e2eUnb64(payload.ciphertext))
...
export async function e2eEncryptDataUrl(dataUrl: string, mk: CryptoKey): Promise<EncryptedPayload> {
  const iv = e2eRandomBytes(12)
  const ct = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, mk, e2eTextEncoder.encode(dataUrl))
  return { ciphertext: e2eB64(new Uint8Array(ct)), nonce: e2eB64(iv) }
}
